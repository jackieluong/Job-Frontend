...
export default function JobSearchCard({job}: JobSearchCardProps) {
    ...
    const handleClickCompanyName = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    const handleClickApply = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.stopPropagation(); // Prevent the click event from bubbling up to the job name
        if(!isAuthenticated) {
    ...
    return (
        <div className='group rounded-xl cursor-pointer min-h-28  bg-white p-3 border-1 border-green-300 transition hover:border-green-600 hover:opacity-90'  >
            <div className='flex gap-5 justify-start pr-1'>
               ...
               <img src={job.companyImg} className='object-contain w-[120px] h-[120px] ' />
                </div>
             ...
             <div className='flex flex-col gap-2 flex-1'>
                ...
                <div className='font-semibold break-words text-lg cursor-pointer hover:text-green-500' onClick={(e) => handleClickJobName(e)}>{job.name}</div>
                    <div className='text-green-700 ml-3'> {displayVNDWithPostfix(job.salaryFrom)} - {displayVNDWithPostfix(job.salaryTo)} </div>
                </div>
                <div className='text-gray-500 break-words cursor-pointer hover:underline text-lg' onClick={(e) => handleClickCompanyName(e)}>{job.companyName} </div>
                <div className='flex gap-2'>
                   <div className='text-sm rounded-xl py-1 px-3 bg-gray-300'>{job.city.join(", ")}</div>
                    <div className='text-sm rounded-xl py-1 px-3 bg-gray-300'>{job.yearOfExperience} năm</div>
                </div>
                ...
                {/* <div className='flex gap-1'>
                    ...
                    <div className='space-x-2 flex items-center'>
                        <Button className='opacity-0 group-hover:opacity-100' onClick={(e) => handleClickApply(e)}>Ứng tuyển</Button>
                        <div className='text-gray-500 group-hover:hidden text-sm'> 
                        {job.updatedAt == null ? <p>Đã đăng {timeAgo(job.createdAt)} </p>
                        : <p> Cập nhật {timeAgo(job.updatedAt)}</p>
                        }
                        
                    </div>
                        <button onClick={(e) => handleClickSave(e)} className="cursor-pointer rounded-full border border-green-500 w-8 h-8 flex items-center justify-center hover:bg-green-500 hover:text-white transition">
