...
import { fetchDistrict, fetchProvinces } from '@/service/locationService';
...
export default function BasicInfo({ jobInfo, setJobInfo}: BasicInfoProps) {
  ...
  useEffect(() => {
    const getProvinces = async () => {
      ...
      const provinces = data?.map((province: Province) =>  {return {code: province.code, name: province.name }});
  ...
  useEffect(() => {
    const getDistricts = async () => {
      if (provinceCode) {
        ...
        const districts = data?.map((district: District) =>  {return {code: district.code, name: district.name }});
  ...
  return (
    <div>
      <div className="py-4 flex flex-col gap-4">
        ...
        <Input name='title' defaultValue={jobInfo.title} onChange={(e) => jobInfo.title = e.target.value} placeholder="Nhập chức danh" required></Input>
      </div>

      <div className="py-4 flex flex-col gap-4">
        <Label className="font-medium">Ngành nghề</Label>
        <Select name='industry' defaultValue={jobInfo.industry} onValueChange={(value) => jobInfo.industry = value} >
          <SelectTrigger>
      ...
      <div className="py-4 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:gap-8">
          <div className='flex flex-col gap-4 lg:flex-1'>
            ...
            <Select name='province' defaultValue={jobInfo.province}  onValueChange={(value) => handleProvinceChange(value) }>
          <SelectTrigger>
          ...
          <div className='flex flex-col gap-4 lg:flex-1'>
            ...
            <Select name='district' defaultValue={jobInfo.district} onValueChange={(value) => jobInfo.district = value} >
          <SelectTrigger>
      ...
      <div className="py-4 flex flex-col gap-4 flex-1">
        ...
        <Input name='quantity' defaultValue={jobInfo.quantity}  onChange={(e) => jobInfo.quantity = Number(e.target.value)} type='number' min={0} placeholder="Nhập số lượng cần tuyển"  />
      </div>
      <div className="py-4 flex flex-col gap-4">
        <Label className="font-medium">Mô tả công việc (nên bao gồm Mô tả Công việc, Yêu cầu công việc)</Label>
        <TextEditor  onChange={handleEditorState} initialContent={jobInfo.description}/>
        <input type="hidden" name="description" value={editorState} />
      </div>
      <div className='flex flex-col gap-4 lg:flex-row lg:gap-8'>
      <div className="py-4 flex flex-col gap-4 flex-1" >
        <Label className="font-medium">Lương tối thiểu</Label>
        <Input name='salaryFrom' defaultValue={jobInfo.salaryFrom} onChange={(e) => jobInfo.salaryFrom = Number(e.target.value)} type='number' min={0} placeholder="Nhập lương tối thiểu"  /> 
      </div>
      <div className="py-4 flex flex-col gap-4 flex-1">
        <Label className="font-medium">Lương tối đa</Label>
        <Input name='salaryTo' defaultValue={jobInfo.salaryTo}  onChange={(e) => jobInfo.salaryTo = Number(e.target.value)} type='number' min={0} placeholder="Nhập lương tối đa"  />
      </div>
      </div>
      <div className="py-4 flex flex-col gap-4 ">
        <Label className="font-medium">Hình thức làm việc</Label>
        <RadioGroup name='jobType' onValueChange={(value) => jobInfo.jobType = value}  defaultValue={jobInfo.jobType} className="flex space-x-8">
        {
      ...
      <div className="py-4 flex flex-col gap-4">
        ...
        <Input name='deadline' onChange={(e) => jobInfo.deadline = e.target.value} defaultValue={jobInfo.deadline} type='date'className='w-1/2 flex justify-between'  placeholder="Nhập hạn nộp hồ sơ" ></Input>
      </div>
    </div>
  );
}
