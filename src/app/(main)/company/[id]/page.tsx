...
function CompanyDetail() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        const dataCompany = await fetchCompanyDetail(Number(id)).then(
          (res) => res.data,
        );
        const jobCompany = await getJobsOfCompany(Number(id));
        const status = await getStatusFollowCompany(Number(id)).then(
          (res) => res.data,
        );
        setFollow(status.follow);
        setCompany(dataCompany);

        totals.current = jobCompany.totalElement;
        pageCount.current = jobCompany.totalPage;
        setJobsOfCompany(jobCompany.data);
      } catch (error) {
              ...
              <div className="cardJob px-4 py-4 flex flex-col gap-2">
                {jobsOfCompany?.map((job) => (
                  <CardJob
                    ...
                    salary={`Từ ${displayVNDWithPostfix(job.salaryFrom)} - ${displayVNDWithPostfix(job.salaryTo)}`}
                    location={job.city}
            ...
            <div className="bg-white rounded-lg mb-4">
              ...
              <div className="px-4 py-4">
                <div className="flex flex-row gap-2 items-center">
                  ...
                  <p className="text-sm font-semibold py-2">Địa chỉ công ty</p>
                </div>
                <p className="text-sm text-gray-700">
                  {company?.address?.location}, {company?.address?.district},{' '}
