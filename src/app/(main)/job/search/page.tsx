...
export default function page(props: pageProps) {
  ...
  const keywordRef = useRef<string | null>(keyword);
  ...
  const [minSalary, maxSalary] =
    salaryRange?.split('-').map((val) => Number(val)) || [];
  const [minExperience, maxExperience] =
    experienceRange?.split('-').map((val) => Number(val)) || [];
      ...
      <div className="lg:w-3/4 mx-auto mt-8 space-y-4">
        ...
        <div className="bg-white rounded-lg p-4">
          <p className="text-gray-600 text-lg">
            Khám phá những công việc phù hợp với bạn
          </p>
          <p className="text-lg text-black mt-1">
            Tuyển dụng{' '}
            <span className="font-bold text-green-600">{total.current}</span>{' '}
            công việc{' '}
            {keywordRef.current && (
              <span className="font-semibold"> {keywordRef.current}</span>
            )}{' '}
            phù hợp
          </p>
        </div>
          ...
          <div className="flex gap-3">
            {sortOptions.map((option, index) => (
              <Button
                ...
                className={`border-gray-200 transition rounded-2xl ${sortBy === option.value ? 'bg-green-200 text-blue-600' : ''}`}
              >
                {option.label}
              </Button>
            ))}
