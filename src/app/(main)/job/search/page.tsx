...
export default function page(props: pageProps) {
    ...
    const [minExperience, maxExperience] = experienceRange?.split('-').map(Number) || [];
    ...
    const fetchJobs = async () => {
        try {
            ...
            const data = await searchJobs(
                currentPage + 1, pageSize, sortBy, false,
                keywordRef.current, jobType, industry, level,
                minExperience, maxExperience, minSalary, maxSalary, city
            );
            total.current = data.totalElement;
            pageCount.current = data.totalPage;
            setJobs(data.data);
    ...
    // Trigger when filter or pagination changes
    useEffect(() => {
        fetchJobs();
    }, [currentPage, pageSize, sortBy, jobType, industry, level, minExperience, maxExperience, minSalary, maxSalary]);

    // Trigger when keyword changes
    useEffect(() => {
        if (keyword) {
            keywordRef.current = keyword;
            setCurrentPage(0);
        ...
        <div className="min-h-screen">
            ...
            <div className="lg:w-3/4 mx-auto mt-8 space-y-4">
                <div className="bg-white rounded-lg p-4">
                    <p className="text-gray-600 text-lg">Khám phá những công việc phù hợp với bạn</p>
                    <p className="text-lg text-black mt-1">
                        Tuyển dụng <span className="font-bold text-green-600">{total.current}</span> công việc 
                        {keywordRef.current && <span className="font-semibold"> "{keywordRef.current}"</span>} phù hợp
                    </p>
                </div>
                ...
                <div className='bg-white flex flex-col lg:flex-row lg:justify-between lg:items-center px-8 py-5'>
                    ...
                    <Button size="lg"><Bell /> Nhận thông báo</Button>
                </div>
            </div>
        </div>
    );
