                ...
                <div className="flex flex-row gap-4 px-24 py-6">
                    ...
                    <div className="left-section basis-7/10 h-auto">
                        ...
                        {loading ? (
                            ...
                            <div>
                            <p className="mb-6">
                                Danh sách <strong>{savedJobs.length}</strong> việc làm đã lưu
                            </p>
                            {savedJobs?.map((job) => (
                                <div key={job.id} className="border border-gray-200 cursor-pointer rounded-lg mb-2">
                                        ...
                                        <div className="right w-full flex flex-col justify-between">
                                            <div className="flex justify-between gap-2">
                                                <div className="flex flex-col">
                                                    <h3 className="text-xl font-semibold mb-1 hover:text-green-600">
                                                        <Link href={`/jobdetail/${job.id}`} className="">
                                                            {job.name}
                                                        </Link>
                                                    </h3>
                                                    <Link href={`/companyDetail/${job.companyId}`} className="text-base text-gray-600 uppercase mb-1">
                                                        {job?.companyName}
                                                    </Link>
                                                </div>
                                                <div>
                                                    <span className="text-green-600 font-semibold text-nowrap">{`Từ ${job.salaryFrom} - ${job.salaryTo} VNĐ`}</span>
                                                </div>
                                            </div>
                                            <div className="flex flex-row gap-2 justify-between">
                                                <div className="flex flex-row flex-wrap gap-2">
                                                    ...
                                                    <div className="h-[fit-content] w-[fit-content] text-xs bg-gray-300 px-1 py-1 rounded-xs">Yêu cầu {job?.yearOfExperience} năm kinh nghiệm</div>
                                                </div>
