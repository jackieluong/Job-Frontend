...
export const statusColorMap: Record<string, string> = {
  ...
  REVIEWED: 'bg-green-100 text-blue-800',
};
...
export const roleColorMap: Record<string, string> = {
  ...
  USER: 'bg-green-400 text-blue-800 p-2 rounded-xl text-sm',
};
...
export const CVStatusColorMap: Record<string, string> = {
  ...
  REJECTED: 'bg-red-400 text-gray-800',
  PENDING: 'bg-yellow-100 text-yellow-800',
  SEEN: 'bg-green-100 text-blue-800',
};

// Create a map where the key is the value and the value is the label
export const industryMap: Record<string, string> = industryOptions.reduce(
  (map, { value, label }) => {
    map[value] = label;
    return map;
  },
